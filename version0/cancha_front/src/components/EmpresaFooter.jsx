...
function EmpresaFooter({ data }) {
  ...
  return (
    <footer className="bg-azul-950 text-gris-200 pt-16 pb-8">
      <div className="mx-auto max-w-7xl px-6 lg:px-8 grid gap-12 md:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="col-span-2">
          <div className="flex items-center gap-2 text-blanco-50 text-lg font-bold">
            {data.logo_imagen && (
              <img
                src={`http://localhost:3000${data.logo_imagen}`}
                alt="Logo"
      ...
      {/* Línea inferior */}
      <div className="mx-10 mt-12 border-t border-verde-600/80 mx-25 pt-6 text-center text-sm text-gris-200">
        Copyright © {new Date().getFullYear()} {data.nombre_sistema || "PlayPass"}
      </div>
    </footer>
  );
}
