...
async function getAllAdministradores() {
  try {
    const query = `
      SELECT a.id_admin_esp_dep, a.fecha_ingreso, a.direccion, p.id_persona, p.nombre, p.apellido, p.telefono, p.correo, p.sexo, p.imagen_perfil
      FROM ADMIN_ESP_DEP a
      JOIN PERSONA p ON a.id_admin_esp_dep = p.id_persona
    `;
...
async function getPersonaByAdminId(id) {
  try {
    const query = `
      ...
      FROM PERSONA p
      JOIN ADMIN_ESP_DEP a ON p.id_persona = a.id_admin_esp_dep
      WHERE a.id_admin_esp_dep = $1
    `;
    const result = await pool.query(query, [id]);
    return result.rows[0];
...
async function getEspaciosByAdminId(id) {
  try {
    const query = `
      SELECT id_espacio, nombre, direccion, descripcion, latitud, longitud, horario_apertura, horario_cierre, id_admin_esp_dep
...
async function createAdministrador(fecha_ingreso, direccion, id_persona) {
  try {
    const query = `
      INSERT INTO ADMIN_ESP_DEP (id_admin_esp_dep, fecha_ingreso, direccion)
      VALUES ($1, $2, $3)
      RETURNING id_admin_esp_dep, fecha_ingreso, direccion
    `;
      ...
      administradores.map(async (admin) => {
        if (admin.imagen_perfil) {
          try {
            ...
            console.warn(`Imagen no encontrada para administrador ${admin.id_admin_esp_dep}: ${admin.imagen_perfil}`);
  ...
  try {
    ...
    if (administrador.imagen_perfil) {
      try {
        ...
        console.warn(`Imagen no encontrada para administrador ${administrador.id_admin_esp_dep}: ${administrador.imagen_perfil}`);
...
router.post('/', verifyToken, checkRole(['ADMINISTRADOR']), crearAdministrador);
...
router.get('/persona/:id_persona', verifyToken, checkRole(['ADMINISTRADOR']), obtenerAdministradorPorIdPersona);
router.get('/:id/persona', verifyToken, checkRole(['ADMINISTRADOR']), obtenerPersonaPorAdminId);
router.get('/:id/espacios', verifyToken, checkRole(['ADMINISTRADOR']), obtenerEspaciosPorAdminId);

router.patch('/:id', verifyToken, checkRole(['ADMINISTRADOR']), actualizarAdministrador);
