...
import { obtenerEncargadoPorId, actualizarEncargado } from "../services/espacioService";
...
function EncargadoEdit() {
  ...
  const [espacio, setEncargado] = useState(null);
  ...
  useEffect(() => {
    ...
    const fetchEncargado = async () => {
      ...
      try {
        ...
        if (!cancelado) setEncargado(resp.data);
      } catch (err) {
        if (!cancelado)
  ...
  const handleSubmit = async (data) => {
    ...
    setError("");
    try {
      await actualizarEncargado(id, data, token);
